...
def signup():
    ...
    if request.method == "POST":
        ...
        if name is None or email is None or password is None or birthday is None  or interests is None or gender is None or phonenumber is None:
       ...
       profile=str1,phonenumber=phonenumber,pastorganizations=pastorganizations,goal_hour = goal_hour, about = about, city = city)
        session.add(user)
        session.commit()
